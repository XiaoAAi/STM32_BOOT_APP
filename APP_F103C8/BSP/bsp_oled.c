...
void TIM2_PWM_init(u16 arr,u16 psc)
{
 ...
 //设置该引脚为复用输出功能,输出 TIM3 CH2 的 PWM 脉冲波形 GPIOB.5 
	...
	u8 Month[3]="04";
	u8 Day[3]="06";
	u8 Hours=0,Minute=0,Second=0;//定义分和秒
//时间显示在OLED 屏幕上
	void Oled_ShowTime()
{
	Second++;
	...
	if(Hours>=24)
		Hours=0;
	if(Hours>12)
		OLED_P8x16Str(20,4,"Pm");
	else
		OLED_P8x16Str(20,4,"Am");
	OLED_P8x16Str(18,0,Year);
	OLED_P8x16Str(50,0,"-");
	OLED_P8x16Str(60,0,Month);
	OLED_P8x16Str(75,0,"-");
	OLED_P8x16Str(85,0,Day);
	OLED_Print_Num(40,4,Hours);
	OLED_P8x16Str(60,4,":");
	OLED_Print_Num(70,4,Minute);
	OLED_Print_Num1(90,5,Second);		
}
...
void Get_date(u8 date[])
{
	...
	memcpy(Day,&date[6],2);
	memcpy(strtemp,&date[8],2);
	Hours=atoi(strtemp);
	memcpy(strtemp,&date[10],2);
	Minute=atoi(strtemp);
...
//{0x3F,0xFF,0x22,0x20,0x22,0x20,0x22,0x20,0x22,0x20,0x20,0x20,0x00,0x20,0x00,0x00},/*"年",0*/

//{0x00,0x01,0x00,0x02,0x00,0x0C,0x7F,0xF0,0x44,0x40,0x44,0x40,0x44,0x40,0x44,0x40},
//{0x44,0x40,0x44,0x40,0x44,0x42,0x44,0x41,0x7F,0xFE,0x00,0x00,0x00,0x00,0x00,0x00},/*"月",1*/

//{0x00,0x00,0x00,0x00,0x00,0x00,0x7F,0xFF,0x41,0x02,0x41,0x02,0x41,0x02,0x41,0x02},
//{0x41,0x02,0x41,0x02,0x41,0x02,0x7F,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},/*"日",2*/
//};
//内部使用用户无需调用
...
void OLED_Init(void)
{
	...
	OLED_RST(1);
	OLED_DLY_ms(200);
	//OLED_set(OLED_SDA_PIN, 1)	;		
	//OLED_SCL(1);
	//gpio_init (OLED_CS_PIN,1,0);
	//OLED_CS(0); //使能信号端，拉低时正常使用
	//gpio_init (OLED_RST_PIN,1,1);
	OLED_RST(0);
	OLED_DLY_ms(200);
	OLED_RST(1);
...
{
	uint8_t *ch1,ch[7];
	if(num<0)   {num = -num;OLED_P6x8Str(x, y, (u8 *)"-");}
	else         OLED_P6x8Str(x, y, (u8 *)" ");
	x+=6;
